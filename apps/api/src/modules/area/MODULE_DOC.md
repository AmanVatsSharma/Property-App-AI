...
**Dependencies:** TypeORM (Area entity), LoggerModule, ConfigService; agent config (AGENT_CONFIG_KEYS) for provider and API keys. No REST/GraphQL in this module.

**APIs:** None. Area is consumed internally by the agent module (get_neighbourhood_score, assess_region, score_property).
